import type { Team, Task, AutomationStatus } from "../data";

// ─── State ──────────────────────────────────────────────────────────────────
let state: Team[] = [];
let initialized = false;
let dragSrc: { ti: number; taski: number } | null = null;

const STATUS_OPTIONS: AutomationStatus[] = ["live", "testing", "planned", "manual"];
...
export async function initTable(): Promise<void> {
  if (!initialized) {
    const raw: Team[] = await fetch("/api/teams").then(r => r.json());
...
function numInput(val: number | undefined, ti: number, taski: number, key: string, opts = ""): string {
  return `<input type="number" value="${val ?? ""}" data-ti="${ti}" data-taski="${taski}" data-key="${key}" ${opts} />`;
}

function textInput(val: string | undefined, ti: number, taski: number, key: string, placeholder = ""): string {
  return `<input type="text" value="${esc(val ?? "")}" placeholder="${esc(placeholder)}" data-ti="${ti}" data-taski="${taski}" data-key="${key}" />`;
}

function statusSelect(val: AutomationStatus, ti: number, taski: number): string {
...
function attachListeners(container: HTMLElement): void {
  ...
  // Status select: also update color live
  container.querySelectorAll<HTMLSelectElement>("select[data-key='automationStatus']").forEach(sel => {
    sel.addEventListener("input", () => {
      sel.style.color = STATUS_COLORS[sel.value as AutomationStatus] ?? STATUS_COLORS.manual;
    });
  });

  // Team header field changes
  container.querySelectorAll<HTMLInputElement>("input[data-field]").forEach(el => {
    el.addEventListener("change", handleTeamChange);
  });

  // Drag-and-drop task reordering
  container.querySelectorAll<HTMLTableCellElement>("td.col-drag[draggable]").forEach(handle => {
  ...
  container.querySelectorAll<HTMLTableRowElement>("tr.tbl-task-row").forEach(row => {
...
function handleTeamChange(e: Event): void {
  ...
  const ti    = parseInt(el.dataset.ti!);
  const field = el.dataset.field as keyof Team;
  const team  = state[ti];
  if (!team) return;
  (team as any)[field] = field === "teamFteWeight" ? parseFloat(el.value) || 0 : el.value;
}

// ─── Structural mutations (re-render after each) ──────────────────────────────
export function tableAddTask(ti: number): void {
  state[ti].tasks.push({
    name: "New task",
    description: "",
    timePercent: 0,
...
export function tableAddTeam(): void {
  state.push({
    name: "New Team",
    lead: "",
    teamFteWeight: 5,
...
export function tableDeleteTask(ti: number, taski: number): void {
  state[ti].tasks.splice(taski, 1);
  renderTable();
}

export function tableDeleteTeam(ti: number): void {
...
// ─── Save ────────────────────────────────────────────────────────────────────
export async function tableSave(): Promise<void> {
  const btn = document.getElementById("tbl-save-btn") as HTMLButtonElement | null;
...
// ─── Expose to window ────────────────────────────────────────────────────────
declare global { interface Window { render?: () => Promise<void>; } }

(window as any).initTable        = initTable;
(window as any).tableSave        = tableSave;
(window as any).tableAddTask     = tableAddTask;
(window as any).tableAddTeam     = tableAddTeam;
(window as any).tableDeleteTask  = tableDeleteTask;
(window as any).tableDeleteTeam  = tableDeleteTeam;
