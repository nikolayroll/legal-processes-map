...
(() => {
  ...
  function treemap_default() {
    ...
    treemap.paddingInner = function(x) {
      return arguments.length ? (paddingInner = typeof x === "function" ? x : constant_default4(+x), treemap) : paddingInner;
    };
    treemap.paddingOuter = function(x) {
      return arguments.length ? treemap.paddingTop(x).paddingRight(x).paddingBottom(x).paddingLeft(x) : treemap.paddingTop();
    };
    treemap.paddingTop = function(x) {
      return arguments.length ? (paddingTop = typeof x === "function" ? x : constant_default4(+x), treemap) : paddingTop;
    };
    treemap.paddingRight = function(x) {
      return arguments.length ? (paddingRight = typeof x === "function" ? x : constant_default4(+x), treemap) : paddingRight;
    };
    treemap.paddingBottom = function(x) {
      return arguments.length ? (paddingBottom = typeof x === "function" ? x : constant_default4(+x), treemap) : paddingBottom;
    };
    treemap.paddingLeft = function(x) {
      return arguments.length ? (paddingLeft = typeof x === "function" ? x : constant_default4(+x), treemap) : paddingLeft;
    };
    return treemap;
  }
  ...
  var gradientScale = sequential(RdYlGn_default).domain([0, 100]);
  var STATUS_FILL = {
    live: "#16a34a",
    testing: "#2563eb",
    planned: "#9333ea",
  ...
  function getCellFill(d, mode) {
    switch (mode) {
      case "standardization":
        return gradientScale(d.data.repetitiveness ?? 50);
      case "status":
        return STATUS_FILL[d.data.automationStatus ?? "manual"];
    }
  }
  var TEXT_PRIMARY = "#0f172a";
  var TEXT_SECONDARY = "#334155";
  var MODE_META = {
  ...
  var tooltip = select_default2("body").append("div").style("position", "absolute").style("display", "none").style("background", "rgba(15,23,42,0.95)").style("color", "#f1f5f9").style("padding", "12px 16px").style("border-radius", "10px").style("font-size", "13px").style("line-height", "1.65").style("pointer-events", "none").style("max-width", "320px").style("box-shadow", "0 4px 24px rgba(0,0,0,0.45)");
  var svg = select_default2("#chart").attr("viewBox", `0 0 ${W} ${H}`).attr("width", "100%").attr("height", "100%");
  var chartArea = svg.append("g").attr("transform", `translate(${MARGIN.left},${MARGIN.top})`);
  var defs = svg.append("defs");
  var gradId = "lg";
  var lg = defs.append("linearGradient").attr("id", gradId).attr("x1", "0%").attr("x2", "100%");
  ...
  function renderLegend(mode) {
    ...
    if (meta.gradientLeft) {
      legendG.append("rect").attr("x", lx).attr("y", ly).attr("width", lw).attr("height", 10).attr("rx", 3).style("fill", `url(#${gradId})`);
      legendG.append("text").attr("x", lx).attr("y", ly + 22).attr("font-size", "10px").attr("fill", "#64748b").text(meta.gradientLeft);
      legendG.append("text").attr("x", lx + lw).attr("y", ly + 22).attr("font-size", "10px").attr("fill", "#64748b").attr("text-anchor", "end").text(meta.gradientRight);
    } else if (meta.items) {
      meta.items.forEach((item, i) => {
        const ix = lx - 10 + i * 55;
        legendG.append("rect").attr("x", ix).attr("y", ly).attr("width", 13).attr("height", 13).attr("rx", 3).attr("fill", item.color);
        legendG.append("text").attr("x", ix + 17).attr("y", ly + 10).attr("font-size", "11px").attr("fill", "#475569").text(item.label);
      });
    }
  }
  var cellFillRects;
  ...
  var volTexts;
  var descTexts;
  var currentMode = "standardization";
  async function render() {
    chartArea.selectAll("*").remove();
    const raw = await json_default("/api/data");
    const root2 = hierarchy(raw).sum((d) => d.value ?? 0);
    treemap_default().size([innerW, innerH]).tile((node, x0, y0, x1, y1) => {
      if (node.depth === 0) {
        const gap = 10;
        const outerPad = 4;
        ...
        dice_default(node, x0, y0, x1, y1);
      } else {
        squarify_default(node, x0, y0, x1, y1);
      }
    }).paddingTop((d) => d.depth === 2 ? 22 : 0).paddingInner((d) => d.depth === 1 ? 6 : 2).round(true)(root2);
    const allTeams = (root2.children ?? []).flatMap((g) => g.children ?? []);
    const leaves = root2.leaves();
    const teamGs = chartArea.selectAll("g.team").data(allTeams).join("g").attr("class", "team");
    teamGs.append("rect").attr("x", (d) => d.x0).attr("y", (d) => d.y0).attr("width", (d) => d.x1 - d.x0).attr("height", (d) => d.y1 - d.y0).attr("rx", 6).attr("fill", "none").attr("stroke", "#334155").attr("stroke-width", 1.5);
    teamGs.append("text").attr("x", (d) => d.x0 + 8).attr("y", (d) => d.y0 + 15).attr("font-size", "12px").attr("font-weight", "700").attr("fill", "#0f172a").text((d) => `${d.data.name}  \xB7  ${d.data.lead ?? ""}`);
    const cells = chartArea.selectAll("g.cell").data(leaves).join("g").attr("class", "cell").style("cursor", "pointer");
    cellFillRects = cells.append("rect").attr("x", (d) => d.x0 + 1).attr("y", (d) => d.y0 + 1).attr("width", (d) => Math.max(0, d.x1 - d.x0 - 2)).attr("height", (d) => Math.max(0, d.y1 - d.y0 - 2)).attr("rx", 4).attr("fill", (d) => getCellFill(d, currentMode));
    cells.on("mousemove", (event, d) => {
      ...
      tooltip.style("display", "block").style("left", `${event.pageX + 14}px`).style("top", `${event.pageY - 28}px`).html(
        `<strong style="font-size:14px">${d.data.name}</strong>` + (d.data.description ? `<div style="font-style:italic;color:#94a3b8;font-size:12px;margin:3px 0 7px">${d.data.description}</div>` : `<br/>`) + `<span style="color:#64748b">Team:</span> ${d.parent?.data.name}<br/><span style="color:#64748b">Time share:</span> <strong>~${d.data.timePercent?.toFixed(0)}%</strong> of ${d.parent?.data.name ?? "team"} time<br/><span style="color:#64748b">Volume:</span> ${volLine}<br/><span style="color:#64748b">Standardization:</span> <strong>${d.data.repetitiveness}%</strong><br/><span style="color:${statusDotColor[status]}">\u25CF ${statusLabel[status]}</span>` + (d.data.note ? `<div style="color:#64748b;font-size:11px;margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.12)">${d.data.note}</div>` : "")
      );
    }).on("mouseleave", () => tooltip.style("display", "none"));
    nameTexts = cells.append("text").attr("x", (d) => d.x0 + 6).attr("y", (d) => d.y0 + 14).attr("font-size", "11px").attr("font-weight", "600").attr("fill", TEXT_PRIMARY).attr("pointer-events", "none").text((d) => {
    ...
    volTexts = cells.append("text").attr("x", (d) => d.x0 + 6).attr("y", (d) => d.y0 + 26).attr("font-size", "10px").attr("fill", TEXT_SECONDARY).attr("pointer-events", "none").text((d) => {
      const w = d.x1 - d.x0, h = d.y1 - d.y0;
      if (w < 50 || h < 30) return "";
      return `~${d.data.timePercent?.toFixed(0)}% of team time`;
    });
    descTexts = cells.append("text").attr("x", (d) => d.x0 + 6).attr("y", (d) => d.y0 + 38).attr("font-size", "9.5px").attr("font-style", "italic").attr("fill", TEXT_SECONDARY).attr("pointer-events", "none").text((d) => {
      const w = d.x1 - d.x0, h = d.y1 - d.y0;
  ...
  function setMode(mode) {
    ...
    currentMode = mode;
    const t = transition().duration(380).ease(cubicOut);
